...
def __init_plugin__(app=None):
    ...
    addmenuitemqt("Load AutoDock DLG File", show_gui)
...
class DLGLoaderGUI(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.file_label = QtWidgets.QLabel("Select .dlg file(s):")
    ...
    def loadFile(self):
        # Use self.selected_files if available, else fallback to text field
        filenames = getattr(self, "selected_files", None)
        if not filenames:
            text = self.file_input.text()
            filenames = [f.strip() for f in text.split(";") if f.strip()]
        if filenames:
            for filename in filenames:
                load_dlg(filename)
        else:
            QtWidgets.QMessageBox.warning(self, "Error", "Please select at least one file.")
